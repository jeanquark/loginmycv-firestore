<template>
    <div class="text-center mt-0 pa-4" style="" v-if="userResume">
        ...
        <v-row no-gutters justify="center" class="pa-3" v-if="userResume">
            <v-alert :value="true" color="warning" icon="mdi-exclamation" outlined v-if="!userResume.education.length > 0">
                        ...
                        <v-expansion-panel-header>
                            <v-row no-gutters align="center">
                                ...
                                <v-btn icon class="mr-2">
                                    <v-icon @click.native.stop="deleteItem(index)" color="error" class="">mdi-delete</v-icon>
                                </v-btn>
	...
	export default {
		...
		computed: {
			loadDynamicComponent() {
				...
				return () => import(`~/components/resume/dynamicTemplatesComponents/${this.userResume.template.id}/EducationComponent`)
			},
			errors() {

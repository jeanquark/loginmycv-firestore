<template>
	...
	>
		<div class="fullpage-container background-color text-color">
			<div
			    class="button-group ma-4"
			    v-if="resume.menus && (resume.education.length > 0 || resume.work_experience.length < 10 || resume.skills.length > 0)"
			>
				<button
				    type="button"
				    :class="{active:index == 0}"
				    @click="moveTo(0)"
				>{{ resume.menus.home ? resume.menus.home : 'Presentation' }}</button>
				<button
				    type="button"
				    :class="{active:index == 1}"
				    @click="moveTo(1)"
				    v-if="resume.education && resume.education.length > 0"
				>{{ resume.menus.education ? resume.menus.education : 'Education'}}</button>
				<button
				    type="button"
				    :class="{active:index == 2}"
				    @click="moveTo(2)"
				    v-if="resume.work_experience && resume.work_experience.length > 0"
				>{{ resume.menus.work_experience ? resume.menus.work_experience : 'Work experience' }}</button>
				<button
				    type="button"
				    :class="{active:index == 3}"
				    @click="moveTo(3)"
				    v-if="resume.skills && resume.skills.length > 0"
				>{{ resume.menus.skills ? resume.menus.skills : 'Skills' }}</button>
				<button
				    type="button"
				    :class="{active:index == 4}"
				    @click="moveTo(4)"
				    v-if="resume.parameters && resume.parameters.show_contact_form"
				>{{ resume.menus.contact ? resume.menus.contact : 'Contact' }}</button>
			</div>
			...
			>
				...
				>
					<v-layout
					    ...
					    align-center
					    class="subpage"
					    style="border: 0px dashed brown;"
					    v-animate="{value: animationEffect}"
					>
						<v-flex
						    xs12
						    class="pa-4"
						...
						>
							...
							>
								...
								<v-flex
								    v-bind="{[`xs12 sm${column}`]: true}"
								    class="text-xs-center secondary-color-background pa-3 rounded-border"
								>
									...
									<v-layout
									    row
									    wrap
									    class="text-big text-xs-left"
									>
										<v-flex
										    xs6
										    class="mb-3"
										    v-if="resume.personal_data.city || resume.personal_data.country"
										>
										...
										<v-flex
										    xs6
										    class="mb-3"
										    v-if="resume.personal_data.email"
										>
											<font-awesome-icon
											    :icon="['fas', 'envelope']"
											    class="icon"
										...
										<v-flex
										    xs6
										    class="mb-3"
										    v-if="resume.personal_data.phone_number"
										>
											<font-awesome-icon
											    :icon="['fas', 'phone']"
											    class="icon"
											/> {{ resume.personal_data.phone_number }}
										</v-flex>
										<v-flex
										    class="mb-3"
										...
										>
											...
											/>
											<span
											    v-for="(nationality, index) in resume.personal_data.nationalities"
											    :key="index"
											>
						...
						<v-flex
						    v-bind="{[`xs12 sm${presentationColumns}`]: true}"
						    class="pa-4 centered"
						    style="border: 0px solid orangered;"
						>
							<div
							    class="text-xs-center"
							...
							>
								<font-awesome-icon
								    ...
								    size="5x"
								    class="social-link"
								    @click="redirectTo(file.downloadUrl)"
								/>
								<br />
								<p>{{ file.name }}</p>
						...
						>
							...
							>
								...
								<v-card-text class="card-text">
									...
									<p
									    class="primary-color accentuate"
						...
						>
							...
							>
								...
								<v-card-text class="card-text">
									<p
									    class="primary-color accentuate"
									    v-if="!work_experience.end_date"
									>{{ work_experience.start_date }}</p>
									<p
									    class="primary-color accentuate"
									    v-else
									>{{ work_experience.start_date }} - {{ work_experience.end_date }}</p>
				...
				>
					...
					>
						...
						<v-flex
						    ...
						    style="height: 100%; border: 0px solid red;"
						>
							...
							>
								<v-flex
								    xs12
								    sm6
								    class="my-4 mx-0 px-3"
								    style="border: 0px dashed orange;"
								    v-for="s in skill"
								    :key="s.name"
								>
									<div
		...
		computed: {
			...
			files() {
				if (this.resume.uploads) {
					return this.resume.uploads.filter(
						file => file.type === "downloadable_file"
					)
				}
			},
			profilePicture() {

                    ...
                    <v-layout justify-center align-center>
                        <div>
                            ...
                            <div style="min-width: 300px;">
                                <card class='stripe-card' :class='{ complete }' stripe='pk_test_mfRXE3nfVNydQzc5zhpTrdoU004DyRbBvo' :options="stripeOptions" @change='complete = $event.complete' v-if="loadedStripe" /><br />
                            </div>
                        </div>
                    </v-layout>
                </v-card-text>
	...
	export default {
		...
		computed: {
			...
			daysRemaining() {
				if (
					this.loadedUser &&
					this.loadedUser.private &&
					this.loadedUser.private.package_valid_until
				) {
					return Math.floor(
						(this.loadedUser.private.package_valid_until -
							moment().unix()) /
			...
			validityDuration() {
				// return 10
				return Math.floor(
					(this.valid_until - moment().unix()) / (60 * 60 * 24)
				)
			}
		},
		methods: {
			...
			calculatedUpgradeCost(pack, daysRemaining) {
				...
				return Math.min(
					pack.price / 100,
					Math.round((pack.price * daysRemaining * 20) / (100 * 365)) / 20
				).toFixed(2)
			},
			...
			async pay() {
				...
				try {
					...
					const process = await axios.post('/stripe-payments', {
						...
						name: `${this.loadedUser.firstname} ${
							this.loadedUser.lastname
						}`
					})
					...
					new Noty({
						type: 'error',
						text:
							'Sorry, an error occured and the payment process could not be fullfilled successfully. No charge incurred.',
						timeout: 5000,
						theme: 'metroui'
					}).show()
				}

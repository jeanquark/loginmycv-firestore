		...
		<v-container>
			...
			<v-layout row wrap justify-center>
				...
				<v-flex xs12 sm6 md4 class="pa-4" v-for="pack in loadedPackages" :key="pack.slug">
					<v-card hover :class="[loadedUser && loadedUser.private && loadedUser.private.package_slug === `${pack.slug}` ? `active-${pack.slug}` : '']">
						<v-img
						...
						<v-card-actions v-if="loadedUser && loadedUser.private">
							...
							<v-layout justify-center v-if="pack.slug === 'classic'">
								...
								<v-layout justify-center row wrap v-if="loadedUser.private.package_slug === 'classic' && daysRemaining <= 60">
									<v-chip label outline color="secondary" text-color="secondary" class="mb-2 px-2 py-4">Current active package.<br />Only {{ daysRemaining }} days remaining.</v-chip><br />
									<v-btn flat color="secondary" @click.stop="selectPackage(pack, daysRemaining)">Renew package!</v-btn>
								</v-layout>

								<v-layout justify-center v-if="loadedUser.private.package_slug === 'advanced' && daysRemaining <= 60">
									<v-btn flat color="secondary" @click.stop="selectPackage(pack, daysRemaining)">Downgrade to classic!</v-btn>
								</v-layout>
							</v-layout>
							...
							<v-layout justify-center v-if="pack.slug === 'advanced'">
								...
								<v-layout row wrap justify-center v-if="loadedUser.private.package_slug === 'advanced' && daysRemaining <= 60">
									<v-chip label outline color="tertiary" text-color="tertiary" class="mb-2 px-2 py-4">Current active package.<br />Only {{ daysRemaining }} days remaining.</v-chip><br />
									<v-btn flat color="tertiary" @click.stop="selectPackage(pack, daysRemaining)">Renew package!</v-btn>
								</v-layout>

								<v-layout justify-center v-if="loadedUser.private.package_slug === 'classic' && daysRemaining > 60">
									<v-btn flat color="tertiary" @click.stop="selectPackage(pack, daysRemaining, calculatedUpgradeCost(pack), )">Upgrade to advanced for €{{ calculatedUpgradeCost(pack, daysRemaining) }}!</v-btn>
								</v-layout>
								<v-layout row wrap justify-center v-if="loadedUser.private.package_slug === 'classic' && daysRemaining <= 60">
									<v-flex xs12 class="text-xs-center">
										<v-btn flat color="tertiary" @click.stop="selectPackage(pack, daysRemaining)">Upgrade to advanced!</v-btn>
									</v-flex>
									<v-flex xs12 class="text-xs-center">
										<small class="tertiary--text">And gain {{ daysRemaining }} days!</small>
									</v-flex>
								</v-layout>
							</v-layout>
						</v-card-actions>
						...
						<v-card-actions v-else>
							<v-layout justify-center>
								<v-btn flat :color="pack.color" @click.stop="selectPackage(pack, daysRemaining)">Pick me!</v-btn>
							</v-layout>
						</v-card-actions>
					</v-card>
				</v-flex>
						    ...
						    <v-list>
						    	<template>
						        	...
						        	<v-list-tile avatar ripple>
						              	...
						              	<v-list-tile-action>
							                <v-list-tile-action-text class="black--text"><b>{{ this.pack.name }}</b></v-list-tile-action-text>
						              	</v-list-tile-action>
						            </v-list-tile>
						            <v-divider ></v-divider>
						            <v-list-tile avatar ripple>
						              	...
						              	<v-list-tile-action>
							                <v-list-tile-action-text class="text-xs-center black--text"><b>{{ this.valid_until | moment('LL') }}</b><br /> (in {{ validityDuration }} days)</v-list-tile-action-text>
						              	</v-list-tile-action>
						            </v-list-tile>
						            <v-divider ></v-divider>
						            <v-list-tile avatar ripple>
						              	...
						              	<v-list-tile-action>
							                <v-list-tile-action-text class="black--text"><b>{{ (this.amount_in_cents / 100).toFixed(2) }}</b></v-list-tile-action-text>
						              	</v-list-tile-action>
						            </v-list-tile>
						        </template>
							</v-list>
		...
		computed: {
			...
			daysRemaining () {
				if (this.loadedUser && this.loadedUser.private && this.loadedUser.private.package_valid_until) {
					return Math.floor((this.loadedUser.private.package_valid_until - moment().add(0, 'month').unix())/(60 * 60 * 24))
					// return Math.floor((this.loadedUser.private.package_valid_until - moment().unix())/(60*60*24))
				}
				return 0
			},
			validityDuration () {
				return ((this.valid_until - moment().unix()) / (60 * 60 * 24))
			}
		},
		methods: {
			displayCost (price) {
